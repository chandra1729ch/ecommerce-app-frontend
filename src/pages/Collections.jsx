 ...
 /**  useEffect(() => {
  ...
  useEffect(() => {
    if (subCategory.length > 0) {
      setFilterProducts(products.filter(item => subCategory.includes(item.subCategory)))
    } else {
      setFilterProducts(products)
    }
  }, [subCategory])
    ...
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t'>
      ...
      <div className="min-w-60">
        <p onClick={() => setShowFilter(!showFilter)} className='my-2 text-xl flex item-center cursor-pointer gap-2'>FILTERS
          <img className={`h-3 my-2 sm:hidden ${showFilter ? 'rotate-90' : ''}`} src={asset.dropdown_icon} alt="" srcset="" />
        </p>
        {/* category filter */}
